...
export default function ProfilePage() {
    ...
    const handleSendUSDC = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!user?.wallet?.address || !amount || parseFloat(amount) <= 0 || !recipient) return;

        const selectedWallet = wallets.find(w => w.address === user.wallet?.address) || wallets[0];
        if (!selectedWallet) {
        ...
        try {
            ...
            const userTokenAddress = await getAssociatedTokenAddress(usdcMint, userPublicKey);
            const recipientTokenAddress = await getAssociatedTokenAddress(usdcMint, recipientPublicKey);
            ...
            if (error.message?.includes("TokenAccountNotFoundError") || error.message?.includes("Account not found")) {
                toast.error("Recipient does not have a USDC token account. Creation not supported in this simple view.");
            } else {
                ...
                <div className="max-w-xl mx-auto space-y-6">
                    ...
                    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-sm">
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                            <div>
                                <div className="text-zinc-500 text-xs font-medium uppercase tracking-wider mb-1">Available Balance</div>
                                <div className={`text-3xl font-black text-zinc-900 dark:text-white transition-all duration-500 ${isRefreshing ? "animate-pulse text-green-500" : ""}`}>
                                    {balance} <span className="text-lg font-bold text-zinc-400">USDC</span>
                                </div>
                            </div>
                            <button
                        ...
                        <div className="mt-4 flex items-center gap-2 p-2 bg-zinc-50 dark:bg-zinc-950 rounded border border-zinc-100 dark:border-zinc-800/50">
                            <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Address</span>
                            <span className="font-mono text-xs text-zinc-600 dark:text-zinc-400 break-all">{user?.wallet?.address}</span>
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-sm">
                        ...
                        <form onSubmit={handleSendUSDC} className="space-y-4">
                            ...
                            <button
                                type="submit"
                                disabled={sending}
                                className={`w-full py-3 bg-[#07C285] hover:bg-[#06a874] text-white rounded-lg font-bold text-sm shadow-sm transition-all cursor-pointer ${sending ? "opacity-70 cursor-not-allowed" : ""}`}
                            >
