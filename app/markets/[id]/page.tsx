...
export default function MarketDetailPage({
    ...
    if (loading) return <div className="space-y-4 p-8 max-w-7xl mx-auto"><div className="h-64 bg-zinc-100 dark:bg-zinc-900 animate-pulse rounded-xl"></div></div>;
        ...
        <div className="min-h-screen bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 font-sans">
            <header className="sticky top-0 z-20 bg-white/80 dark:bg-zinc-950/90 backdrop-blur-md border-b border-zinc-100 dark:border-zinc-800">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
                    <Link href="/" className="flex items-center gap-2 text-[#07C285] hover:text-[#06a874] transition-colors py-2">
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    ...
                    <div className="lg:col-span-8 space-y-10">
                        <div className="flex gap-6 items-start">
                            {market.image_url && (
                                <img src={market.image_url} alt={market.title} className="w-20 h-20 rounded-full object-cover shadow-sm hidden md:block" />
                            )}
                            <div>
                                <h1 className="text-2xl md:text-3xl font-bold tracking-tight mb-2">{market.title}</h1>
                        ...
                        <div className="border-t border-zinc-100 dark:border-zinc-800 pt-8">
                            ...
                            <div className="prose dark:prose-invert prose-zinc max-w-none text-sm leading-relaxed text-zinc-600 dark:text-zinc-400">
                                {market.description || "No specific details provided for this market."}
                            </div>
                        </div>
                    ...
                    <div className="lg:col-span-4 relative">
                        <div className="sticky top-24">
                            <div className="flex items-center gap-3 mb-4">
                                {market.image_url && <img src={market.image_url} className="w-10 h-10 rounded-full object-cover" />}
                                <div>
                                    <div className="text-xs font-bold text-zinc-400">{action === "buy" ? "Buying" : "Selling"} {outcome === "yes" ? "Yes" : "No"}</div>
                                    <div className="font-bold leading-tight">{market.title.slice(0, 40)}...</div>
                                </div>
                            </div>
                                ...
                                <div className="px-6 pb-6 space-y-4">
                                    ...
                                    <div className="space-y-1">
                                        <label className="text-xs font-bold text-zinc-400 uppercase">
                                            {orderType === "limit" || action === "sell" ? "Shares" : "Amount"}
                                        </label>
                                        <div className="relative">
                                            ...
                                            {orderType === "market" && action === "buy" && (
                                                <span className="absolute right-0 top-1/2 -translate-y-1/2 text-zinc-400 font-bold pr-2">USDC</span>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-2 pt-2">
                                        <div className="flex justify-between text-sm">
                                            ...
                                            <span className="font-mono font-bold">{orderType === "limit" ? limitPrice.toFixed(2) : (outcome === "yes" ? "0.78" : "0.22")}¢</span>
                                        </div>
                                        <div className="flex justify-between text-sm">
                                            <span className="text-zinc-500">Estimated cost</span>
                                            <span className="font-mono font-bold text-zinc-900 dark:text-white">
                                                {amount
                                                    ? `$${(orderType === "market" && action === "buy" ? parseFloat(amount) : parseFloat(amount) * (orderType === "limit" ? limitPrice : (outcome === "yes" ? 0.78 : 0.22))).toFixed(2)}`
                                                    : "$0.00"}
                                            </span>
                                        </div>
                                        <div className="flex justify-between text-sm">
                                            <span className="text-zinc-500">Payout if {outcome === "yes" ? "Yes" : "No"}</span>
                                            <span className="font-mono font-bold text-green-600 dark:text-green-400">
                                                {action === "buy"
                                                    ? (amount ? `$${(orderType === "market" && action === "buy" ? parseFloat(amount) / (outcome === "yes" ? 0.78 : 0.22) : parseFloat(amount)).toFixed(2)}` : "$0.00")
                                                    : "N/A (Selling)"
                                                }
                                            </span>
                                        </div>
                                    ...
                                    >
                                        {!user ? "Log in to Trade" : tradeLoading ? "Processing..." : `${action === "buy" ? "Buy" : "Sell"} ${outcome === "yes" ? "Yes" : "No"}`}
                                    </button>
                                </div>
                            </div>
                        </div>
                ...
                <SplitMergeModal
                    isOpen={isModalOpen}
                    onClose={() => { setIsModalOpen(false); setOrderType("market"); }}
